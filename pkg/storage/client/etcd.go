...
type Client interface {
	...
	WatchList(ctx context.Context, key string) clientv3.WatchChan
	Close() error
}
...
func (c *client) Create(ctx context.Context, key string, value []byte) error {
	resp, err := c.c.Txn(ctx).
		If(clientv3.Compare(clientv3.ModifiedRevision(key), "=", 0)).
...
func (c *client) Update(ctx context.Context, key string, revision int64, value []byte) error {
	resp, err := c.c.Txn(ctx).
		If(clientv3.Compare(clientv3.ModifiedRevision(key), "=", revision)).
		Then(clientv3.OpPut(key, string(value))).
...
func (c *client) Watch(ctx context.Context, key string) clientv3.WatchChan {
	return c.c.Watch(ctx, key)
}

func (c *client) WatchList(ctx context.Context, key string) clientv3.WatchChan {
	return c.c.Watch(ctx, key, clientv3.WithPrefix())
}
